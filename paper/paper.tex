% !TeX document-id = {5ae73978-d7a8-4a15-a272-3b0597ff2241}
...
\titlerunning{Modeling and Simulation of Load Balancing Strategies}

%Single insitute
%\author{ \and Firstname Lastname}
...
% Multiple insitutes - ALTERNATIVE to the above
 \author{
Ren\'e Caspart\inst{1} \and
Patrick Firnkes\inst{2} \and
Manuel Giffels\inst{1} \and
Anne Koziolek\inst{2} \and
G\"unter Quast\inst{1} \and
Ralf Reussner\inst{2} \and
Maximilian Stemmer-Grabow\inst{2}
 }


%
%If there are too many authors, use \authorrunning
\authorrunning{R. Caspart et al.}
%
\institute{
	Institute for Experimental Particle Physics (ETP) \and
...
\Cref{related} describes different attempts to simulate the WLCG or parts of it. However, they showed scalability issues or the simulation does not obtain the necessary detail level to simulate effects on working nodes that required to evaluate load balancing strategies.
...
This paper is organized as follows: \Cref{sec:palladio} presents the Palladio simulator as a foundation of the simulation. \Cref{sec:process} gives a overview on the simulation process.\Cref{sec:model} describes how different elements of the computing jobs and the computing resources are represented in our model and the implementation of the necessarily extension of Palladio. \Cref{sec:param} explains how we determine the model parameters and the automated model creation. The validation is presented in \Cref{validation} and \Cref{related} puts our work in context with the related work. In \Cref{outlook} possible design decisions and how they could be modeled and simulated are presented. Finally, \Cref{conlusion} gives a conclusion on this paper.
...
While processing, the job uses the specified number of cores to process the CPU demand. When the job finishes, it releases the job slots. All of these parameters are specified as a probability function and are extracted from measured performance data as described in \Cref{sec:param}.
...
After adding the delay the simulated throughput matches the measured one accurately, as \Cref{throughput} shows.
...
The simulated CPU efficiency is lower than the measured one, as shown in \Cref{utilization}. The reason for this is likely missing jobs in the job data. Because of that is the calibrated throughput is too low resulting in a lower utilization.
...
However, after they initial design was developed, the project was discontinued in 2000. 
...
However, both of them are missing graphical model editors which makes the models of the computing jobs are time-consuming to create \cite{simgrid}\cite{cloudsim}. The support of graphical editors is a big advantage of Palladio and increases its usability and efficiency. 
