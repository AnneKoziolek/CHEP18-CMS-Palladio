...
\begin{abstract}
	...
	As first step to meet both challenges, a more efficient utilization of the grid and coping with the rising complexity of the system, a simulation for the WLCG should be created. \todo{Vorherigen Satz umbauen, Einschub im vorderen Teil schwer verständlich} There is currently no simulation existing helping the operators of the grid to make the correct decisions while optimizing the load balancing strategy.
...
To solve both challenges, a more efficient usage of the grid and coping with the rising complexity of the system\todo{Einschub umformulieren}, a simulation for the WLCG should be created.\todo{``we aim to'' statt ``should'', auch im Abstract}
...
The model and simulation are created using the Palladio simulator, see \Cref{sec:palladio} for details on Palladio. This leads to the research question which modifications are needed so that Palladio can be used to evaluate load balancing strategies for the CMS computing jobs, because the simulation of load balancing strategies is a new use case for Palladio \todo{lieber more high level: ``can we simulate?''. Eher: This paper presents prototype simulation model and extensions of Palladio?}. To realise it we created an extension for the simulator that enables us to simulate load balancing strategies and furthermore, we created an automated model parameter analysis and model creation, in order to increase the usability of the simulation.
...
\label{sec:model}\todo{Grafik?}
This section describes how the computing jobs and working nodes are modeled using Palladio.
The component model describes each type of CMS computing job, such as analysis or reprocessing jobs, with their resource demands. These demands are the CPU demand, the I/O demand, the number of used CPU cores, the required job slots, and the number of processed events.
...
\label{sec:param}\todo{Links zu den Projekten einfügen, wo langfristig archivieren? Zendodo?}
...
is recommended. \todo{Formulierung nicht passiv. Positiver: How to aggregate is future work?}
